dofile(LockOn_Options.script_path.."Scripts/UHF_Radio/Indicator/RemotePilot/defs.lua")
...
function AddMaskLevel(object, level)
    ...
    if level == nil then
        level = UHF_DEFAULT_LEVEL
    end
	object.level			= level
	--object.level			= UHF_DEFAULT_LEVEL + 1
...
function AddElement(object)
	...
	object.h_clip_relation = h_clip_relations.COMPARE -- COMPARE -- REWRITE_LEVEL
	--object.h_clip_relation = h_clip_relations.DECREASE_IF_LEVEL
	object.level			= UHF_DEFAULT_LEVEL
	--object.level			= UHF_DEFAULT_LEVEL + 1
...
function create_box(PosX, PosY, SizeX, SizeY, material, parent, clipRelation, clipLevel, transparent)
	...
	box.level			= UHF_DEFAULT_LEVEL
    
	--box.controllers = {{"hsd_visible"},  {"ecmd_bright"}}

	--box.level			= UHF_DEFAULT_LEVEL + 1
	--Add(box)
...
function clip_box_in(PosX, PosY, SizeX, SizeY)
	...
	__clip_box[3] = SizeX
	__clip_box[4] = SizeY
	create_box(PosX, PosY, SizeX, SizeY, UHF_GREEN, nil, h_clip_relations.INCREASE_IF_LEVEL ,UHF_DEFAULT_LEVEL, false)
end
...
function clip_box_out(PosX, PosY, SizeX, SizeY)
	...
	if SHOW_DEBUG_CLIP_BOXES ~= nil then
		create_box(PosX,PosY,SizeX,SizeY,UHF_TRANSPARENT, nil, h_clip_relations.DECREASE_IF_LEVEL ,UHF_DEFAULT_LEVEL + 1, true)
	end

	create_box(PosX, PosY, SizeX, SizeY, UHF_GREEN, parent, h_clip_relations.DECREASE_IF_LEVEL ,UHF_DEFAULT_LEVEL + 1, false)
end
...
function create_origin()
	local grid_origin	 = CreateElement "ceSimple"
        ...
        grid_origin.controllers = {{"arc159freq_visible_remote_pilot"}}
