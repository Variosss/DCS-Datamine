...
function startup_stage1()
	...
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd193,	value = 0.0, message = _("Parking brake - Set")})
	...
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd250,	value = 1.0, message = _("Battery - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd287,	value = 1.0, message = _("Warning Horn - On")})
	--  Master failure warning light RESET	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd265,	value = 1.0})	
	
	push_start_command(1.0, 	{message = _("Seat height - Adjust")})
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd362,	value = 1.0, message = _("Oxygen - Check")})
	...
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd319,	value = 0.0})	
	...
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd54,	    value = 0.0})	
	...
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd57,	    value = 1.1})	
	push_start_command(0.1,		{device = devices.MAIN,	action = devCmds.Cmd58,	    value = 1.0})	
	...
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd201,	value = 1.0})
	...
	push_start_command(0.15,	{device = devices.MAIN,	action = devCmds.Cmd71,	    value = 0.0})
	...
	push_start_command(0.15,	{device = devices.MAIN,	action = devCmds.Cmd197,	value = 0.0})
	...
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd586,	value = 0.0})
	--- MATRA 550 missile switch - Off
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd582,	value = 0.0})
	...
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd200,	value = 0.0})
	...
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd178,	value = 0.0, message = _("Canopy - Closed")})
	...
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd58,	    value = 0.0, message = _("LP main cock switch - Open")})	
	push_start_command(0.0,		{device = devices.MAIN,	action = devCmds.Cmd57,	    value = -0.1})	
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd67,	    value = 1.0, message = _("L/H LP pump switch - On")})
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd66,	    value = 1.0, message = _("R/H LP pump switch - On")})	
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd54,	    value = 1.0, message = _("Ignition/Ventilation selector switch - To the other plug")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd55,	    value = 1.1, message = _("Start button cover - Open")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd56,	    value = 1.0, message = _("Start button - Depress (for 1 sec approx.)")})
	push_start_command(1.2,		{device = devices.MAIN,	action = devCmds.Cmd56,	    value = 0.0})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd55,	    value = -0.1})	
	
	--- Between 300 and 600 rpm - advance the throttle to IDLE
	push_start_command(6.0,		{device = devices.MAIN,	action = devCmds.Cmd49,	    value = 11.65 / 90.0, message = _("Advance throttle to IDLE")})	
	
	push_start_command(23.0,	{message = _("AFTER START CHECKS")})

	----- After start checks

	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd123,	value = 1.0, message = _("High-lift device selector switch - Normal")})	
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd638,	value = 0.0, message = _("Combat flap lever - In")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd459,	value = 1.0, message = _("UHF radio - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd122,	value = 1.0, message = _("Slat/Flap lever - Full aft")})
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd157,	value = 1.0, message = _("V/UHF radio - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd159,	value = 1.0, message = _("25W - 5W switch - 5W")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd255,	value = -1.0, message = _("Inverter selector switch - Reset")})
	...
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd20,	    value = 0.0})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd540,	value = 0.3333, message = _("IFF - Sby")})

	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd555,	value = -1.0, message = _("Navigation indicator - As required")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd230,	value = 1.0, message = _("Standby horizon switch - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd202,	value = 1.0, message = _("Electro-pump switch - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd224,	value = 1.0, message = _("Probe heater switch - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd443,	value = 1.0, message = _("Radar detector switch - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd580,	value = 0.5, message = _("Sight selector - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd613,	value = 0.5, message = _("Radar selector - Standby")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd237,	value = 0.333, message = _("Heading and vertical reference system control switch - Gm")})
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd239,	value = 1.0, message = _("Emergency gyromagnetic compass switch - On")})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd337,	value = 0.666, message = _("TACAN - Rec")})
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd340,	value = 1.0, message = _("VOR/ILS - On")})
	
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd376,	value = 1.0, message = _("Air conditioning - On")})
		
	-- Pull the knob
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd234,	value = 1.0, message = _("Standby horizon - Uncage")})
	-- Rotate the knob to the middile position. Effectively it will uncage the gyro
	push_start_command(0.5,		{device = devices.MAIN,	action = devCmds.Cmd236,	value = 0.5})
	-- Release the knob
	push_start_command(0.1,		{device = devices.MAIN,	action = devCmds.Cmd235,	value = 0.0})
		
	push_start_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd650,	value = 1.0, message = _("Ejection handle safety pin - Removed (out of sequence)")})	
...
-- Need to add this to confirm that servos reset correctly (after C.AERO advisory goes off)
	push_start_command(6.0,		{device = devices.MAIN,	action = devCmds.Cmd20,	    value = 1.0})	
...
function shut_down()
	...
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd650,	value = 0.0, message = _("Ejection handle safety pin - Installed (out of sequence)")})
		
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd459,	value = -1.0, message = _("UHF radio - Off")})
	
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd157,	value = -1.0, message = _("V/UHF radio - Off")})
	...
	push_stop_command(0.5,		{device = devices.MAIN,	action = devCmds.Cmd236,	value = 1.0})
	-- Release the knob - it will remain in the 'caged' position
	push_stop_command(0.5,		{device = devices.MAIN,	action = devCmds.Cmd235,	value = 0.0})
	...
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd224,	value = 0.0, message = _("Probe heater switch - Off")})
	...
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd237,	value = 0.0, message = _("Heading and vertical reference system control switch - Off")})
	
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd239,	value = 0.0, message = _("Emergency gyromagnetic compass switch - Off")})
	...
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd57,	    value = 1.1})
	push_stop_command(1.0,		{device = devices.MAIN,	action = devCmds.Cmd58,	    value = 1.0, message = _("LP main cock switch - Off")})	
