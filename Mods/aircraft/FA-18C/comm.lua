...
g_crew_items[#g_crew_items  + 1] = { name = _('Wheel chocks'), submenu = menus['Wheel chocks']}
...
g_crew_items[#g_crew_items  + 1] = { name = _('Ground Air Supply'), submenu = menus['Ground Air Supply']}
g_crew_items[#g_crew_items  + 1] = { name = _('Salute!'), command = sendMessage.new(Message.wMsgLeaderGroundGestureSalut, true)}
g_crew_items[#g_crew_items  + 1] = { name = _('Request Launch'), command = sendMessage.new(Message.wMsgLeaderGroundRequestLaunch, true)}

local function helmetEvent(is_NVG)
	return specialEvent({type = COMMAND_CHANGE_COCKPIT_EQUIPMENT, device = is_NVG})
end
...
menus['Change helmet-mounted device'] =
{
	...
	items = {
		[1] = {name = _('Setup JHMCS'),	command = helmetEvent(COCKPIT_DEVICE_HMS) },
		[2] = {name = _('Setup NVG'),	command = helmetEvent(COCKPIT_DEVICE_NVG) },
	}
}
g_crew_items[#g_crew_items  + 1] = { name = _('Change helmet-mounted device'), submenu = menus['Change helmet-mounted device']}