...
local texture_red = MakeMaterial("arcade.tga",{255, 0, 0, 255})
...
local pitch_stick_movement_negative = 60 * k
--[[
...
local tick = Copy(roll_scale)
      if i == 2 then tick.vertices    = {{0, -line_width},{0,  line_width},{3.0*line_width ,  line_width},{3.0*line_width , -line_width}}
      else           tick.vertices    = {{-3.0*line_width, -line_width},{-3.0*line_width,  line_width},{ 0 ,  line_width},{ 0 , -line_width}}   end
...
for i = 1,2 do
	...
	local 	wheel_brake_actual                = CreateElement "ceMeshPoly"
			...
			wheel_brake_actual.material	      = "MFCD_CICU_INIT_LBLUE"
