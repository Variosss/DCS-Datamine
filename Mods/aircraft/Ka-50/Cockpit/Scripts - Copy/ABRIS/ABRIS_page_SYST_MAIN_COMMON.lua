...
local elem_size = 0.05

local columns = 0.6 / elem_size
local rows    = 0.8 / elem_size
local border  = 0.05
math.randomseed(1246568734)
for i=1,columns+1 do
    for j=1,rows do
     local  elem	          = CreateElement "ceMeshPoly"
        elem.material         = "GREEN"
        elem.init_pos		  = {(i-1)*elem_size --[[+ elem_size/2==]] - 0.3, 
                                -(j-1)*elem_size --[[- elem_size/2--]] + 0.4}                               
        elem.primitivetype    = "triangles"
        elem.parent_element   = frame_2.name
        elem.indices          = box_ind
        elem.vertices         = {{border * elem_size            ,-border*elem_size},
                                 {elem_size - border * elem_size,-border*elem_size},
                                 {elem_size - border * elem_size,-elem_size + border * elem_size},
                                 {border * elem_size            ,-elem_size + border * elem_size}}
        elem.controllers      = {{"sist_screen_simulation_color",0,math.random(),math.random(),math.random()}}
        Add(elem)
        use_mipfilter(elem)
    end
end
...
txt_memory_status			     = CreateElement "ceStringPoly"
txt_memory_status.name           = "txt_memory_status"
txt_memory_status.material       = "font_Ka-50_ABRIS_GREEN"
txt_memory_status.init_pos		 = {-0.17*x_size,0,0}
txt_memory_status.alignment      = "RightTop"
txt_memory_status.stringdefs     = abris_font[2]
txt_memory_status.formats		 =  {LOCALIZE("ОСН.ПРОЦЕССОР").."-"..LOCALIZE("ДА").."\n"..LOCALIZE("РПЗУ").." %i"..LOCALIZE("МБ").."\n","   %i%%\n",LOCALIZE("ОЗУ").." %i"..LOCALIZE("МБ").."\n","   %i%%"}
txt_memory_status.controllers		   =  {{"get_EPROM_size",0},
									   {"get_free_EPROM_size_percent",1},
									   {"get_RAM_size",2},
									   {"get_free_RAM_size_percent",3}}
txt_memory_status.parent_element   = up_line.name
Add(txt_memory_status)
use_mipfilter(txt_memory_status)
...
txt_hardware_status.init_pos		 = {-0.17*x_size,0,0}
txt_hardware_status.value            = ""
...
txt_hardware_status4.parent_element   = frame_4.name
txt_hardware_status4.init_pos         = {0.5*x_size,-0.64*y_size,0}
Add(txt_hardware_status4)

txt_hardware_off_status			     	 = CreateElement "ceStringPoly"
txt_hardware_off_status.name             = "txt_hardware_off_status"
txt_hardware_off_status.material         = "font_Ka-50_ABRIS_BLUE"
txt_hardware_off_status.init_pos		 = {-0.3*x_size,0,0}
txt_hardware_off_status.value            = ""
txt_hardware_off_status.alignment        = "RightTop"
txt_hardware_off_status.stringdefs       = abris_font[2]
txt_hardware_off_status.parent_element   = up_line.name
Add(txt_hardware_off_status)
use_mipfilter(txt_hardware_off_status)

txt_hardware_off_status2		     	 = Copy(txt_hardware_off_status)
txt_hardware_off_status2.init_pos		 = {0.9*x_size,-0.07*y_size,0}
txt_hardware_off_status2.value           = "\n\n\n\n\n\n\n\n\n\nARINC"
txt_hardware_off_status2.material         = "font_Ka-50_ABRIS_BLUE"
Add(txt_hardware_off_status2)
use_mipfilter(txt_hardware_off_status2)

txt_hardware_off_status3		     	 = Copy(txt_hardware_off_status2)
txt_hardware_off_status3.value           = "\n\n\n\n\n\n\n\n\n\n \nTX RX\n 0  0\n 1  1\n 2  2\n 3  3"
Add(txt_hardware_off_status3)

txt_hardware_off_status4		     	  = Copy(txt_hardware_off_status2)
txt_hardware_off_status4.value            = "ARINC-429"
txt_hardware_off_status4.parent_element   = frame_4.name
txt_hardware_off_status4.init_pos         = {0.5*x_size,-0.64*y_size,0}
Add(txt_hardware_off_status4)

txt_hardware_off_status5		     	 = Copy(txt_hardware_off_status2)
txt_hardware_off_status5.value           = "\n\n\n\n\n\n\n   1234\n\n        6\n"
Add(txt_hardware_off_status5)
...
for i = 1,4 do
	...
	if i == 4 then
		lines[i].material        = "INDICATION_ABRIS_BLUE"
	end
	lines[i].width           = 0.004 
...
lines[11].material        = "INDICATION_ABRIS_BLUE"
