...
local function helmetEvent(is_NVG)
	return specialEvent({type = COMMAND_CHANGE_COCKPIT_EQUIPMENT, cabin = 0, device = is_NVG})
end
...
menus['Change helmet-mounted device'] =
{
	...
	items = {
		[1] = {name = _('Setup HMS'),	command = helmetEvent(COCKPIT_DEVICE_HMS) },
...
menus['Select power source'] =
{
	...
	items = {
		[1] = {name = _('Request turbo gear'),	command = specialEvent({type = COMMAND_CHANGE_POWER_SOURCE, power_source = POWER_SOURCE_TURBO_GEAR}) },
		[2] = {name = _('Use regular launch'),	command = specialEvent({type = COMMAND_CHANGE_POWER_SOURCE, power_source = POWER_SOURCE_REGULAR_LAUNCH}) },
	}
}
