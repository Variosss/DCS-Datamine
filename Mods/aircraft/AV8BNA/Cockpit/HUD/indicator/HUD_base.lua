...
local material_visible		= false

-- First HUD FOV
local TFOV_Name			= "TFOV"
local TFOV_radius		=  170 * MilToDI(HUD_SCALE)	-- mils
local TFOV_collimated	= true

if hud_repeater then
	material_visible	= false
	mask_material_TFOV	= MakeMaterial(nil, {0, 255, 255, 10})
	TFOV_Name			= "TFOV_" .. HUD_ID
	TFOV_radius			=  210 * MilToDI(HUD_SCALE)
	TFOV_collimated		= false
	
	--local log = require("log")
	--log.info(string.format("HUD BASE:: HUD REPEATER: %s, GLASS LEVEL %f, DEFAULT LEVEL %f, SCALE %f, TFOV %f", HUD_ID, GLASS_LEVEL, HUD_DEFAULT_LEVEL, HUD_SCALE, TFOV_radius))
end
...
total_field_of_view.collimated			= TFOV_collimated
