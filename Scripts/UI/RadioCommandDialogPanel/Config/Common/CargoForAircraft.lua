...
local function getCargos()
	return data.pUnit:getNearestCargosForAircraft()
end

local function buildCargos(self, menu)
	local t_cargos = getCargos()
	menu.items[mainMenuPos] = buildCargosMenuForAircraft(t_cargos, _('All Cargos'), { name = _('Cargos'), submenu = menus['Cargos']}, data.pUnit)
end
