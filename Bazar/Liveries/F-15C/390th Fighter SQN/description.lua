livery = {
	{"f15nose", 0 ,"f15_nose",true};
	{"f15nose", 3 ,"f15_decol_NG",true};
	{"f15bottom", 0 ,"f15_bottom",true};
	{"f15bottom", 3 ,"f15_decol_NG",true};
	{"f15centr", 0 ,"f15_centr",true};
	{"f15wingl", 0 ,"f15_wing_l",true};
	{"f15wingl", 3 ,"f15_decol_NG",true};
	{"f15wingr", 0 ,"f15_wing_r",true};
	{"f15stab", 0 ,"f15_stab_v",true};
	{"f15stab", 3 ,"f15_decol_NG",true};
	{"f15numbers", 0 ,"f15_numbers",true};
	{"pilot_F15_patch", 0 ,"pilot_F15_patch_390_sqdn",true};
	{"pilot_F15_00_body", 0 ,"pilot_f15_00_b",true};
	{"f15PTB", 0 ,"f15_PTB",true};
}

countries = {"USA"}