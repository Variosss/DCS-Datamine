livery = {
	{"f15nose", 0 ,"greek_f15_nose",true};
	{"f15nose", 3 ,"f15_decol_empty",false};
	{"f15bottom", 0 ,"greek_f15_bottom",true};
	{"f15bottom", 3 ,"f15_decol_empty",false};
	{"f15centr", 0 ,"greek_f15_centr",true};
	{"f15wingl", 0 ,"greek_f15_wing_l",true};
	{"f15wingl", 3 ,"f15_decol_empty",false};
	{"f15wingr", 0 ,"greek_f15_wing_r",true};
	{"f15wingr", 3 ,"f15_decol_empty",false};
	{"f15stab", 0 ,"greek_f15_stab_v",true};
	{"f15stab", 3 ,"f15_decol_empty",false};
  {"f15numbers", 0 ,"f15_numbers",true};
	{"pilot_F15_patch", 0, "greek_pilot_f15_patch_65a_sqdn", true};
  {"pilot_F15_00_body", 0 ,"pilot_f15_00_a",true};
	{"f15PTB", 0 ,"f15_PTB",true};
}
name = "Hellenic Airforece - Aegean Ghost (Fictional)"
countries = {"GRC",}
