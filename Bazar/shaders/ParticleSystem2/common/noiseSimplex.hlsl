...
// Other corners
	...
	// int2 i1 = int2(1, 0) * (1 - xLessEqual) + int2(0, 1) * xLessEqual;
	int2 i1 = (x0.x<= x0.y) ? int2(0, 1) : int2(1, 0);
...
{
	static const float contrast = 2;
	float result = 0.0f, amplitude, frequency = 0;

	[unroll]
	for ( int i = 0; i < 3; ++i ) 
	{       
		//amplitude = 1*0.75;
		amplitude = pow(0.5,i+1);
		result += (contrast*snoise(uv*exp2(i))*0.5+0.5) * amplitude;
		//frequency += 1;
	}
	return result;
}
