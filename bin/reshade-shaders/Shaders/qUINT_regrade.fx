...
//85 and 52 are perfect divisors of 255 and 33 is a common LUT size
//Values picked after perceptual luma so that G > R > B in precision. 
//Results in superior quality than 52x52x52 and is incidentally almost
//the same pixel count. 
//Why these values and not 63, 65 or other common sizes? 
//65 is an issue on DX9 (if using a horizontal layout) and other mappings
//are annoying to deal with. And when picking divisors of 255, a neutral LUT
// is _actually_ neutral and does not introduce a subtle color cast.
...
texture2D ReGradeStateTex	             { Width = 1;   Height = 1;     Format = R8;  	};
sampler2D sReGradeStateTex		         { Texture = ReGradeStateTex;   };


texture2D LUTAtlas	                    { Width = LUT_DIM_X;   Height = LUT_DIM_Y * NUM_SECTIONS;     Format = RGBA32F;  	};
texture2D LUTFlattened	                { Width = LUT_DIM_X;   Height = LUT_DIM_Y;                    Format = RGB10A2;  	};
...
float3 tetrahedral_volume_sampling(sampler volumesampler, float3 pn, int3 volume_size, int atlas_idx)
{
...
//todo optimize?
    ...
    int2 texturecoord000 = int2(corner000.x + corner000.z * volume_size.x, corner000.y + volume_size.y * atlas_idx);
    int2 texturecoordmax = int2(cornermax.x + cornermax.z * volume_size.x, cornermax.y + volume_size.y * atlas_idx);
    int2 texturecoordmin = int2(cornermin.x + cornermin.z * volume_size.x, cornermin.y + volume_size.y * atlas_idx);
    int2 texturecoord111 = int2(corner111.x + corner111.z * volume_size.x, corner111.y + volume_size.y * atlas_idx);
...
VSOUT VSLUT(in uint id : SV_VertexID)
{
    VSOUT o;
    VS_FullscreenTriangle(id, o.vpos, o.uv);
    o.vpos *= tex2Dfetch(sReGradeStateTex, 0).x;
    return o;
}
...
void PSMakeLUTs(in VSOUT i, out float4 o : SV_Target0)
{
    int gradeop = uint(floor(i.vpos.y)) / uint(LUT_DIM_Y) + 1; //0 is "skip"
...
void PSLUTFlatten(in VSOUT i, out float4 o : SV_Target0)
{
    ...
    for(int op = 0; op < NUM_SECTIONS; op++)
    {
        ...
        col = tetrahedral_volume_sampling(sLUTAtlas, col, int3(LUT_DIM_R, LUT_DIM_G, LUT_DIM_B), curr_op - 1);
    }
...
{    
    ...
    col = tetrahedral_volume_sampling(sLUTFlattened, col, int3(LUT_DIM_R, LUT_DIM_G, LUT_DIM_B), 0).rgb; //no atlas, no LUT
...
float4 VSSetState(in uint id : SV_VertexID) : SV_Position
{
    return float4(0,0,0,1); //kingeric be like float2(0,1).xxxy
}
void PSSetState(in float4 vpos : SV_Position, out float o : SV_Target0)
{ 
    o = tex2Dfetch(sLUTAtlas, 0).w == 0 //if texture is empty, generate it
      || tex2Dfetch(sLUTFlattened, 0).w == 0
      || OVERLAY_ACTIVE; //update if tweaking
}
...
>
{
    pass
	{
        VertexShader = VSSetState;
		PixelShader = PSSetState;
        RenderTarget = ReGradeStateTex;
