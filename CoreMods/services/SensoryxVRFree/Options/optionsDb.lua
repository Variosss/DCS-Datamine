...
--Trunk can't start on Win7 , error "loading module 'edSensoryxVRFree'
local ok, VRFree = pcall(require, "edSensoryxVRFree")
...
local function checkOtherTracker()
	...
	if saved_dlg ~= nil then
		...
		if is_used_controllers_in_VR_options == true then
			...
			local other_trackers = VRFree.checkTrackers()
