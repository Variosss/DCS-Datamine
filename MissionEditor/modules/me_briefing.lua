...
function show(b)
	if managerModulesME.callBack_getBriefing == nil then
		if not window then 
			create_()			
		end    
		...
		if not ModBrief_ then 
			ModBrief_ = managerModulesME.callBack_getBriefing()
			ModBrief_.create(x_, y_, w_, h_, bEditImages_onChange)
		end    
		
		ModBrief_.show(b, MissionModule.mission)		
	end
